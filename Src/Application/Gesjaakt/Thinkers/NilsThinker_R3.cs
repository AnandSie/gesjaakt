...
public class NilsThinker_R3 : IGesjaaktThinker
{
    ...
    public bool lowValue(IGesjaaktReadOnlyGameState gameState, int value)
    {
        var ga = gameState.OpenCardValue - gameState.AmountOfCoinsOnTable;
        return ga < value;
    }

    public bool freeCoins(IGesjaaktReadOnlyGameState gameState)
    {
    ...
    private static bool IsStreetPotential(IGesjaaktReadOnlyGameState gameState, int cardsBetweenAllowed)
    {
    ...
    public static bool CardPlayed(IGesjaaktReadOnlyGameState gameState, int cardValue)
    {
        foreach (var player in gameState.Players)
        {
            if (player == gameState.PlayerOnTurn)
    ...
    public static bool AnchorCard(IGesjaaktReadOnlyGameState gameState)
    {
...
&& gameState.AmountOfCoinsOnTable >= gameState.Players.Count() - 2)
    ...
    public bool pushGreed(IGesjaaktReadOnlyGameState gameState)
    {
